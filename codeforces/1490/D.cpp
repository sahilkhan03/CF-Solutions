...
void solve() {
	...
	function<void(int, int, int, int)> dfs = [&](int u, int l, int r, int d) {
		debug(u, l, r, d);
		ans[inv[u]] = d;
		if (inv[u] + 1 <= r) dfs(rm.query(inv[u] + 1, r), inv[u] + 1, r, d + 1);
		if (l <= inv[u] - 1)dfs(rm.query(l, inv[u] - 1), l, inv[u] - 1, d + 1);
	};
	dfs(rm.query(0, n - 1), 0, n - 1, 0);
	cout << ans << endl;
}
