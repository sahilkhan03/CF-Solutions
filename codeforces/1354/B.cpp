...
int main()
{
    ...
    while (t--)
    {
        string s;
        cin >> s;
        vector<vl> pre(3, vl(s.size() + 1, 0));
        loop(i, s.size())
        {
            pre[0][i + 1] += pre[0][i] + (s[i] == '1');
            pre[1][i + 1] += pre[1][i] + (s[i] == '2');
            pre[2][i + 1] += pre[2][i] + (s[i] == '3');
        }
        // cout << pre << endl;
        ll i = 0, j = 1, ans = 1e18;
        while (j <= s.size())
        {
            while (pre[0][j] - pre[0][i] and pre[1][j] - pre[1][i] and pre[2][j] - pre[2][i])
                ans = min(ans, j - i), i++;
            j++;
        }
        if (ans == 1e18)
