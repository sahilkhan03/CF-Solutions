...
void solve() {
	...
	vl v(n); cin >> v;
	string t = "hard";
	vector<vl> dp(5, vl(n + 1, 1e18));
	dp[0][0] = 0;
	for(int inc = 0; inc < 5; inc++) {
		for(int j = 1; j <= n; j++) {
			if(inc < 4 and t[inc] == s[j - 1]) {
				dp[inc][j] = dp[inc][j - 1] + v[j - 1];
			} 
			else dp[inc][j] = dp[inc][j - 1];
			if(inc and t[inc - 1] == s[j - 1]) 
				dp[inc][j] = min(dp[inc][j - 1], dp[inc - 1][j - 1]);
		}
	}
